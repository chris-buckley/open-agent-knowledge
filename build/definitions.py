"""Generate one OAK definition per authorable language construct."""
...
TARGET = ROOT / "generated" / "definitions"


def _filename(model: type) -> str:
    return slug(model.__name__) + ".oak.md"


def document(model: type) -> str:
    """Return one construct definition as canonical XML-grouped OAK."""
    ...
    node = Node(
        ...
        constants=[
            ...
            *[
            ...
            Constant(id="syntax-reference", value="generated/oak.ebnf"),
            Constant(
                id="grammar",
                form="text",
...
def documents() -> dict[str, str]:
    """Return every generated OAK construct definition."""
    return {_filename(model): document(model) for model in AUTHORABLE_MODELS}


def write() -> Path:
    """Write the complete generated definition set."""
    write_generated({TARGET / name: text for name, text in documents().items()},
                    root=ROOT / "generated", owned=TARGET)
    return TARGET
