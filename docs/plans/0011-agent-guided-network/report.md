...
## Compression continuation, 6 September 2026

The user authorised testing the more-compressed-AI hypothesis and maintaining a learning index on the same branch. Protocol and index commit `35317423f42720bfdc50637b3109f80409e07882` precedes the frozen scored study. [The compression report](../../../experiments/agent-guided-network/results/compression-run/REPORT.md) records exact 416-to-144 folding, seven live proposals (six accepted), a three-shared-gain task replacement, and twelve isolated final export checks. The fixed four-candidate search selects identical matrices. The nearest-key algorithm solves the task without learned coefficients. No general-AI compression or agent-superiority conclusion is justified.

[The learning index](../../../experiments/agent-guided-network/LEARNINGS.md) consolidates all three studies with stable findings and limitations. Its maintenance is routed through experiments/AGENTS.md. The new compact numerical profile, canonical OAK nodes, data/control/proposal/evaluation commands, measured bytes, full raw evidence, and replay are contained within the existing experiment. No dependency, core OAK grammar, or old numerical source is changed.

Local compilation, generation, both full repository verification entry points, all 59 experiment tests, and complete replays of all three studies passed. [Verification evidence](../../../experiments/agent-guided-network/results/compression-run/verification.json) records actual checks, logs, the initial combined-command timeout, and the successful separate runs. Frozen code hashes identify the measured implementation independently of its later publication commit. The pending publication/CI task is not completed by these local checks alone.


### Compression publication and concurrent plan allocation

Commit `b3c2a645cf229323ac34afd93d1a90cc2955e65b` publishes the compression implementation, learned OAK nodes, learning index, and measured evidence. Its tree `068fa91b6028820dd6caed997021efa21a2ad467` matches the locally checked publication subset. [Experiment workflow 33998939391](https://github.com/chris-buckley/open-agent-knowledge/actions/runs/33998939391) passed all three studies' tests and complete numerical replays.

[Repository workflow 33998939472](https://github.com/chris-buckley/open-agent-knowledge/actions/runs/33998939472) failed only at `RuntimeError: persistent plan check failed: duplicate plan number 0010`. Concurrent main revision `68aab91e7b4c807b91865642c3e9b2fec60956a0` allocated 0010 to Python conventions. After reading the current root and documentation owners and checking occupied plan numbers, this experiment record moves to `0011-agent-guided-network`. Current navigational links are repaired; the historical path descriptions above are retained. No frozen numerical source, study setting, scientific measurement, or validation rule is changed. The corrected revision's CI evidence is recorded after it is observed.


### Verified compression delivery

The corrected full tree `3fcfdd57d5c921a8080eccea779fa9abfc8aa7c4`, published as `ff3e9bbb2fdee1963ef0a734823adf34c76c1eec`, exactly matches the checked local source. All 117 relative document links resolve, both complete repository entry points pass, and all 24 frozen scientific source identities remain unchanged. [Repository workflow 33999351173](https://github.com/chris-buckley/open-agent-knowledge/actions/runs/33999351173) and [experiment workflow 33999351078](https://github.com/chris-buckley/open-agent-knowledge/actions/runs/33999351078) both completed successfully. [Pinned CI evidence](evidence/ci.json) records the tested revision and scope.

Compression delivery is complete, and P11.02 is marked passed. The final record update only changes this delivery report, the task-state record, and their CI evidence. It does not alter the measured implementation, trained parameters, study, or raw results. Its own workflow outcome remains separately visible on PR 14 rather than inferred from the preceding revision. P04.02 remains open for the unperformed broad ablations and matched total-cost accounting. Main is unchanged by this experiment.
