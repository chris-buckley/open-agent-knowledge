...
### Current State
The design starts from `cd1f8aed74b24f8515a3e176972e9f2cbcb53e5a` on `experiment/agent-guided-network`. The current request authorises an experiment directory and a complete synthesis in `EXPERIMENT.md`, not training runs or paid model use. The [experiment specification](../../../experiments/agent-guided-network/EXPERIMENT.md) owns the proposed architecture.
...
- Limitation: runtime implementation, final numeric study settings, and experimental budgets are not yet supplied.
- Limitation: local repository cloning failed because the container could not resolve github.com; repository-wide execution requires another verified execution surface.

## 2. Mission

Record the complete experiment design and its execution gates on the requested branch, then implement and evaluate it only after the corresponding authorisation.

Task: Deliver the experiment design package in this change and retain uniquely identified tasks for subsequent execution.
Purpose: Make the user's learning intent precise, testable, and independent of inference-time agents.
End state: A committed design package with clear evidence status; eventual implementation is judged separately against numerical and scientific criteria.
...
- [ ] Key task: P02.01 Freeze module topology, equations, operation identities, parameter shapes, and OAK schema contracts.
- [ ] Key task: P02.02 Implement canonical node authoring, inline parameters, immutable snapshots, and the restricted host adapter.
- [ ] Key task: P02.03 Implement deterministic task generation, numerical execution, and basic validity and no-agent tests.
...
- [ ] Key task: P03.01 Implement observation, proposal, candidate, evaluation, and decision records with revision-aware acceptance.
- [ ] Key task: P03.02 Implement direct-edit and behavioural-proposal fitting treatments with numerical and resource bounds.
- [ ] Key task: P03.03 Implement independent whole-network evaluation, stale-candidate rejection, combined-update tests, and retained-incumbent rollback.
...
Transition trigger: A complete learning cycle and its negative cases pass; scored experiments remain gated by an approved resource budget.

### Phase 4: Run controlled comparisons
Objective: Test engineering improvement and the added value of agents separately.
- [ ] Key task: P04.01 Run a separate pilot and freeze splits, seeds, metrics, effect thresholds, acceptance rules, tolerances, and budgets before scored comparisons.
- [ ] Key task: P04.02 Execute the declared baselines and semantic, diagnostic, and coordination ablations with complete cost accounting.
- [ ] Key task: P04.03 Freeze selected candidates and evaluate the sealed final test set with uncertainty and failure reporting.
...
- [ ] Key task: P05.01 Export the supported numerical graph with complete parameters, preprocessing, decoding, and source identity.
- [ ] Key task: P05.02 Test clean offline loading, source-to-export parameter identity, numerical and decision equivalence, and forbidden-dependency rejection.
- [ ] Key task: P05.03 Report engineering and scientific verdicts separately and record the measured limits before any scaling claim.
...
- Timeline: design requested on 5 September 2026; no training start or delivery estimate is implied.
- Boundaries: this request delivers the design package; future phases do not authorise themselves.
...
| Numerical runtime | One restricted adapter | Future implementation | PENDING |
| Agent and compute budget | To be approved before scored runs | Repository owner | REQUESTED |
| Sealed benchmark data | One versioned study split | Future deterministic generator | PENDING |
...
Reporting: The current report covers design delivery, not completed training. A draft review may be opened to execute existing repository CI; no merge is authorised.

| Decision | Authority | Escalation |
| --- | --- | --- |
| Create branch and design package | User request of 5 September 2026 | Repository owner |
| Begin implementation | Separate repository-owner authorisation | Repository owner |
...
### Acknowledgement
The implementing agent records the requested intent in the design package. Documentation readiness is not implementation authorisation; future phases remain open and gated. No acknowledgement by another party is claimed.
