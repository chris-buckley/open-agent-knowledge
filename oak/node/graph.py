"""Root graph checks for ids, typed references, and process control flow."""
...
def _validate_value(
    ...
    if isinstance(value, ConstantValue):
        ...
        interface = _target(registry, process, value.interface, Interface)
...
def _static_value(
    ...
    if isinstance(value, LiteralValue):
        return value.value
    if isinstance(value, ConstantValue):
        return _target(registry, process, value.constant, Constant).value
...
def _condition_result(
    ...
    if (
        ...
        and isinstance(
            left,
            (ConstantValue, StateValue, InterfaceValue, BindingValue),
        )
        and left == right
    ):
        equal = True
        ...
        if left_value is _STATIC_MISSING or right_value is _STATIC_MISSING:
            return None
        equal = _json_equal(left_value, right_value)
    return equal if step.condition.operator == "equals" else not equal
...
def _validate_steps(
    ...
    for step in steps:
        if isinstance(step, Act):
            ...
            _validate_value(registry, process, step.value)
            ...
            _validate_value(registry, process, step.condition.left)
            _validate_value(registry, process, step.condition.right)
            result = _condition_result(registry, process, step)
            if result is False:
                raise PydanticCustomError(
                    "dead_process_branch",
            ...
            _target(registry, process, step.process, Process)
...
def validate_graph(root: Node) -> None:
    """Reject duplicate ids, invalid references, and invalid process graphs."""
    registry: dict[str, Node | Entry] = {}
    ...
    triggers = [trigger for node in nodes for trigger in node.triggers]
    duplicate_whens = sorted(
        when
        for when, count in Counter(trigger.when for trigger in triggers).items()
        if count > 1
    )
    if duplicate_whens:
        raise PydanticCustomError(
            "duplicate_trigger_when",
            "tree repeats trigger when values: {values}",
            {"values": ", ".join(duplicate_whens)},
        )
    ...
    processes = [process for node in nodes for process in node.processes]
    for process in processes:
