...
def _resolve_value(
    ...
    if isinstance(value, ConstantValue):
        _target_document, constant = graph.entry(
            document,
            value.constant,
            Constant,
        )
...
def _parallel(
    ...
    prepared = [
        {
            binding.placeholder: _resolve_value(
                graph,
                document,
                binding.value,
                state,
...
def _run_steps(
    ...
    for step in steps:
        if isinstance(step, Act):
            ...
            _state_document, entry = graph.entry(
                document,
                step.state,
                State,
            )
            ...
            emissions.append(
                Emission(
                    interface=graph.display_target(
                        document,
                        "interface",
                        identifier,
                    ),
...
def _active_interfaces(
    ...
    for target, values in arrival.interfaces.items():
        document, interface = graph.entry(
            graph.root,
            target,
            Interface,
        )
