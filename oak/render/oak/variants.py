...
def _entries(
    ...
    """Return one part tag pair, empty when it has no entries."""
    if not bodies:
        return f"<{tag}>\n</{tag}>"
    return element(
        tag,
...
def node_xml(node: Node) -> str:
    """Render one node in the fixed OAK arrangement."""
    parts = [
        _entries(
            "instructions",
