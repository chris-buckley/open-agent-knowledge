...
SURFACES = (
    ...
    _surface("constant-inline", Constant, "<ID> AS <SCHEMA_ID>.<PLACEHOLDER>: <VALUE>", rendered=("id", "schema_id", "placeholder", "value"), fixed=("part", "form"), part="constants", when=(("form", "inline"),)),
    _surface("constant-text", Constant, "<ID> AS <SCHEMA_ID>.<PLACEHOLDER>: TEXT<<\n<VALUE>\n>>", rendered=("id", "schema_id", "placeholder", "value"), fixed=("part", "form"), part="constants", when=(("form", "text"),)),
    _surface("constant-json", Constant, "<ID> AS <SCHEMA_ID>.<PLACEHOLDER>: JSON<<\n<VALUE>\n>>", rendered=("id", "schema_id", "placeholder", "value"), fixed=("part", "form"), part="constants", when=(("form", "json"),)),
    _surface("constant-csv", Constant, "<ID> AS <SCHEMA_ID>.<PLACEHOLDER>: CSV<<\n<VALUE>\n>>", rendered=("id", "schema_id", "placeholder", "value"), fixed=("part", "form"), part="constants", when=(("form", "csv"),)),
    _surface("constant-yaml", Constant, "<ID> AS <SCHEMA_ID>.<PLACEHOLDER>: YAML<<\n<VALUE>\n>>", rendered=("id", "schema_id", "placeholder", "value"), fixed=("part", "form"), part="constants", when=(("form", "yaml"),)),
    ...
    _surface("state", State, "<ID> AS <SCHEMA_ID>.<PLACEHOLDER>: <VALUE>", rendered=("id", "schema_id", "placeholder", "value"), fixed=("part",), part="state"),
    ...
    _surface("act-native", Act, 'ACT input="<INPUT>" output="<OUTPUT>": <INSTRUCTION> (<INPUTS>) -> <OUTPUTS>', rendered=("input", "output", "instruction", "inputs", "outputs"), fixed=("kind",), omitted=("tool",), when=(("tool", None),)),
    _surface("act-tool", Act, 'ACT TOOL "<TOOL>" input="<INPUT>" output="<OUTPUT>": <INSTRUCTION> (<INPUTS>) -> <OUTPUTS>', rendered=("tool", "input", "output", "instruction", "inputs", "outputs"), fixed=("kind",), when=(("tool", _PRESENT),)),
    _surface("step-set", Set, "SET <STATE> = <VALUE>", rendered=("state", "value"), fixed=("kind",)),
    _surface("step-emit", Emit, "EMIT <INTERFACE> (<BINDINGS>)", rendered=("interface", "bindings"), fixed=("kind",)),
    _surface("step-if", If, "IF <CONDITION>:\nTHEN:\n  <THEN>\nELSE:\n  <OTHERWISE>", rendered=("condition", "then", "otherwise"), fixed=("kind",)),
    _surface("step-call", Call, "CALL <PROCESS> (<INPUTS>) -> <OUTPUTS>", rendered=("process", "inputs", "outputs"), fixed=("kind",)),
