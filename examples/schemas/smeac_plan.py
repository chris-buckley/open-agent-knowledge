...
situation_clauses = [
    where("OPERATING_ENVIRONMENT", Type(of='string'), NonEmpty(),
          description="one to three sentences on the domain and scope of operations"),
    where("CURRENT_STATE", Type(of='string'), NonEmpty(),
          description="one to three sentences of factual current assessment"),
    where("OBSTACLE", Type(of='string'), NonEmpty(),
          description="the name of a challenge, competitor, blocker, or risk"),
    where("OBSTACLE_ASSESSMENT", Type(of='string'), NonEmpty(),
          description="impact, likelihood, and probable course of the obstacle"),
    where("HIGHER_INTENT", Type(of='string'), NonEmpty(), description="the overarching goal this plan supports"),
    where("ADJACENT_EFFORTS", Type(of='string'), NonEmpty(),
          description="related parallel initiatives and their relevance"),
    where("SUPPORTING_RESOURCES", Type(of='string'), NonEmpty(),
          description="available assets, teams, tools, or capabilities"),
    where("ASSUMPTION", Type(of='string'), NonEmpty(),
          description="one condition assumed true, validated before or during execution"),
    where("CONSTRAINT", Type(of='string'), NonEmpty(),
          description="one must or must-not restriction imposed by leadership or policy"),
    where("LIMITATION", Type(of='string'), NonEmpty(),
          description="one capability or resource shortcoming that restricts options"),
]

mission_clauses = [
    where("MISSION_STATEMENT", Type(of='string'), Lines(min=1, max=1),
          description="one present-tense active-voice sentence answering who, what, when, where, and why"),
    ...
    where("COMPARISON_AUTHORITY", Type(of='string'), OneOf(values=list(ComparisonAuthority)),
          description="required means an accepted completion criterion, illustrative means guidance only"),
    where("COMPARISON_CURRENT", Type(of='string'), NonEmpty(),
          description="the observed starting state, an explicitly absent artifact, or a linked specimen with literal examples preserved in fences"),
    where("COMPARISON_DESIRED", Type(of='string'), NonEmpty(),
          description="the intended result as prose, a fenced specimen, or a linked file with agreed formatting preserved"),
    where("COMPARISON_ACCEPTANCE", Type(of='string'), NonEmpty(),
          description="what must match, what may vary, and how to verify it, or the teaching purpose of an illustrative example"),
]
...
execution_clauses = [
    ...
    where("PHASE_SUCCESS_CRITERIA", Type(of='string'), NonEmpty(),
          description="measurable conditions that end the phase"),
    where("TRANSITION_TRIGGER", Type(of='string'), NonEmpty(),
          description="the event that signals the next phase, mission complete for the last phase"),
    ...
    where("OPERATING_GUIDELINES", Type(of='string'), NonEmpty(),
          description="guidelines governing actions, decisions, and interactions"),
    where("RISK_MITIGATION", Type(of='string'), NonEmpty(), description="identified risks and their mitigations"),
    where("CONTINGENCY_CONDITION", Type(of='string'), NonEmpty(),
          description="one specific adverse event or deviation"),
...
logistics_clauses = [
    ...
    where("RESOURCE_STATUS", Type(of='string'), OneOf(values=['AVAILABLE', 'REQUESTED', 'PENDING', 'AT_RISK', 'UNAVAILABLE']),
          description="the provisioning status"),
    where("SUPPLY_PLAN", Type(of='string'), NonEmpty(),
          description="one to three sentences on how consumable resources are sourced and distributed"),
    where("TRANSPORTATION_PLAN", Type(of='string'), NonEmpty(),
          description="one to three sentences on how assets and deliverables move between stages"),
    where("SUSTAINMENT_PLAN", Type(of='string'), NonEmpty(),
          description="one to three sentences on how the operation is maintained over its duration"),
    where("ROLLBACK_PLAN", Type(of='string'), NonEmpty(),
          description="one to three sentences on how to revert or recover if execution fails"),
]
...
command_clauses = [
    ...
    where("REPORTING_REQUIREMENT", Type(of='string'), NonEmpty(),
          description="one expected report, metric, or status update"),
    ...
    where("ESCALATION_AUTHORITY", Type(of='string'), NonEmpty(),
          description="who to escalate to when the authority is unavailable"),
]
