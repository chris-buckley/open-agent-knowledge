...
ACT TOOL "math.multiply" input="schema.scaling" output="schema.scaled-balance": Multiply <BALANCE> by <FACTOR> and round to 2 decimals to produce <SCALED_BALANCE>. (BALANCE=$BALANCE, FACTOR=$FACTOR) -> SCALED_BALANCE
</process>
...
ACT Reflect on <BALANCE> reaching <TARGET> and produce <REFLECTION>. (BALANCE=$state.current-balance, TARGET=$TARGET) -> REFLECTION
CALL process.scale-balance (BALANCE=$state.reflection-target, FACTOR=$constant.reflection-step) -> SCALED_BALANCE
