...
<triggers>
trigger.title-requested.event := "A title needs review."
trigger.title-requested.source := interface.title-input
trigger.title-requested.process := process.review-title
</triggers>

<processes>
<process id="review-title" name="Review title" input="schema.title" output="schema.review">
ACT input="schema.title" output="schema.review": Assess <TITLE> under the title policy and produce <VERDICT> and <REASON>. (TITLE=$TITLE) -> VERDICT, REASON
